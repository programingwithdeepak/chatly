...
console.log(error)
  ...
  return (
    ...
    >
      <div
        className="fixed bottom-5 right-5 z-[9999] w-[60px] h-[60px] rounded-full bg-[#20c7ff] flex justify-center items-center shadow-lg cursor-pointer hover:scale-105 transition-all"
      ...
      <div className="w-full h-[50%] overflow-auto flex flex-col gap-[20px] items-center mt-[20px]">
        {otherUsers?.map((user) => (
          <div
            ...
            onClick={() => dispatch(setSelectedUser(user))}
          >
            <div className="relative rounded-full shadow-gray-500 bg-white shadow-lg flex justify-center items-center mt-[1px]">
              <div className="w-[60px] h-[60px]   rounded-full overflow-hidden flex justify-center items-center ">
                <img src={user.image || dp} alt="" className="h-[100%]" />
              </div>
              {onlineUsers?.includes(user._id) && (
